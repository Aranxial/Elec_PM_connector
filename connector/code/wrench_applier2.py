...
class ForcePublisher(Node):
    ...
    def listener_callback(self, msg):
        ...
        for i in range(len(position_data)):
            for j in range(i + 1, len(position_data)):
                link_1, pos_1 = position_data[i]
                link_2, pos_2 = position_data[j]
                ...
                Thread(target = self.apply_wrench_to_link, args=(link_1, force_vector,)).start()
                Thread(target = self.apply_wrench_to_link, args=(link_2, force_vector_,)).start()
    ...
    def calculate_force(self, pos_1, pos_2, distance):
        ...
        k = 200000.0  # Adjust this constant to control force magnitude
    ...
    def apply_wrench_to_link(self, link_name, force_vector):
        ...
        c = f'-p "entity: {{name: \'robot{link_name[-2]}::{link_name}\', type: LINK}}, '
        d = f'wrench: {{force: {{x: {force_vector[0]}, y: {force_vector[1]}, z: {force_vector[2]}}}, '
        e = f'torque: {{x: 0.0, y: 0.0, z: 1.0}}}}"'
        command = a + b + c + d + e
        print(command)

        self.get_logger().info(f'Applying wrench to {link_name} with force vector {force_vector}')
